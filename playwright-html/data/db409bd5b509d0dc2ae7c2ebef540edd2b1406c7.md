...
- Name: api\get-token\get-token.combination.spec.ts >> Get Token B2B combination and validation priority >> GT-CMB-005 - valid headers and invalid partnerId / Valid headers and invalid partnerId
- Location: tests\api\get-token\get-token.combination.spec.ts:176:9

# Error details

```
Error: GT-CMB-005: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-CMB-005: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
