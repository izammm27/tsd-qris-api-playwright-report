...
- Name: api\get-token\get-token.body-negative.spec.ts >> Get Token B2B body negative and raw body contract >> GT-BDY-049 - Duplicate partnerId key using raw JSON / Duplicate partnerId key using raw JSON
- Location: tests\api\get-token\get-token.body-negative.spec.ts:16:9

# Error details

```
Error: GT-BDY-049: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-BDY-049: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
