...
- Name: api\get-token\get-token.header-negative.spec.ts >> Get Token B2B header negative >> GT-HDR-059 - client key extremely long safe value / Client key extremely long safe value
- Location: tests\api\get-token\get-token.header-negative.spec.ts:16:9

# Error details

```
Error: GT-HDR-059: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 404 / GT-HDR-059: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 404
```
