...
- Name: api\get-token\get-token.combination.spec.ts >> Get Token B2B combination and validation priority >> GT-CMB-001 - invalid client key and invalid signature / Invalid client key and invalid signature
- Location: tests\api\get-token\get-token.combination.spec.ts:176:9

# Error details

```
Error: GT-CMB-001: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 404 / GT-CMB-001: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 404
```
