...
- Name: api\get-token\get-token.body-negative.spec.ts >> Get Token B2B body negative and raw body contract >> GT-BDY-038 - valid partnerId dengan one character modified / Valid partnerId with one character modified
- Location: tests\api\get-token\get-token.body-negative.spec.ts:16:9

# Error details

```
Error: GT-BDY-038: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-BDY-038: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
