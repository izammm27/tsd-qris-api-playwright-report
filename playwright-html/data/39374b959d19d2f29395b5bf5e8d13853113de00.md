...
- Name: api\get-token\get-token.combination.spec.ts >> Get Token B2B combination and validation priority >> GT-CMB-009 - signature valid for client key A but header client key B / Signature valid for client key A but header client key B
- Location: tests\api\get-token\get-token.combination.spec.ts:176:9

# Error details

```
Error: GT-CMB-009: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 401; Actual: 404 / GT-CMB-009: HTTP status based on expected responseCode mismatch. Expected: 401; Actual: 404
```
