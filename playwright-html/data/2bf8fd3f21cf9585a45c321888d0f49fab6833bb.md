...
- Name: api\get-token\get-token.header-negative.spec.ts >> Get Token B2B header negative >> GT-HDR-054 - tidak dikenal UUID client key / Unknown UUID client key
- Location: tests\api\get-token\get-token.header-negative.spec.ts:16:9

# Error details

```
Error: GT-HDR-054: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 404 / GT-HDR-054: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 404
```
