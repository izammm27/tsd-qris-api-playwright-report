...
- Name: api\get-token\get-token.combination.spec.ts >> Get Token B2B combination and validation priority >> GT-CMB-006 - tidak dikenal client key and signature from tidak dikenal private key / Unknown client key and signature from unknown private key
- Location: tests\api\get-token\get-token.combination.spec.ts:176:9

# Error details

```
Error: GT-CMB-006: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 404 / GT-CMB-006: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 404
```
