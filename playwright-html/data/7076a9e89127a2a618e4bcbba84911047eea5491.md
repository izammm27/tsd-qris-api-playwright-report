...
- Name: api\get-token\get-token.signature-negative.spec.ts >> Get Token B2B signature negative >> GT-SIG-017 - signature dengan Bearer prefix / Signature with Bearer prefix
- Location: tests\api\get-token\get-token.signature-negative.spec.ts:252:9

# Error details

```
Error: GT-SIG-017: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-SIG-017: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
