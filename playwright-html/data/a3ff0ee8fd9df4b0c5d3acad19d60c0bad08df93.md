...
- Name: api\get-token\get-token.contract.spec.ts >> Get Token B2B security and robustness >> GT-SEC-005 - partnerId dengan long safe string does not cause server error / partnerId with long safe string does not cause server error
- Location: tests\api\get-token\get-token.contract.spec.ts:162:9

# Error details

```
Error: GT-SEC-005: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-SEC-005: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
