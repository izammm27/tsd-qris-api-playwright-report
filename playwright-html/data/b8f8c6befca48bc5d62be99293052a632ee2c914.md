...
- Name: api\get-token\get-token.contract.spec.ts >> Get Token B2B security and robustness >> GT-SEC-004 - partnerId dengan SQL-like text does not cause server error / partnerId with SQL-like text does not cause server error
- Location: tests\api\get-token\get-token.contract.spec.ts:162:9

# Error details

```
Error: GT-SEC-004: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-SEC-004: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
