...
- Name: api\get-token\get-token.signature-negative.spec.ts >> Get Token B2B signature negative >> GT-SIG-015 - signature encoded as hex instead of Base64 / Signature encoded as hex instead of Base64
- Location: tests\api\get-token\get-token.signature-negative.spec.ts:252:9

# Error details

```
Error: GT-SIG-015: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-SIG-015: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
