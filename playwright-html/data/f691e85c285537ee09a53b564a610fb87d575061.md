...
- Name: api\get-token\get-token.header-negative.spec.ts >> Get Token B2B header negative >> GT-HDR-062 - client key literal null string / Client key literal null string
- Location: tests\api\get-token\get-token.header-negative.spec.ts:16:9

# Error details

```
Error: GT-HDR-062: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 404 / GT-HDR-062: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 404
```
