...
- Name: api\get-token\get-token.combination.spec.ts >> Get Token B2B combination and validation priority >> GT-CMB-003 - tidak dikirim client key and tidak dikirim signature / Missing client key and missing signature
- Location: tests\api\get-token\get-token.combination.spec.ts:176:9

# Error details

```
Error: GT-CMB-003: responseMessage tidak sesuai TSD untuk responseCode 4007302. Expected responseMessage: Missing Mandatory Field {X-CLIENT-KEY}; Actual responseMessage: Invalid Mandatory Field {X-CLIENT-KEY} / GT-CMB-003: responseMessage does not match TSD for responseCode 4007302. Expected responseMessage: Missing Mandatory Field {X-CLIENT-KEY}; Actual responseMessage: Invalid Mandatory Field {X-CLIENT-KEY}
```
