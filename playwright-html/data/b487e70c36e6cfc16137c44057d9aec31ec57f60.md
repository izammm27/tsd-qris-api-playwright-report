...
- Name: api\get-token\get-token.contract.spec.ts >> Get Token B2B security and robustness >> GT-SEC-002 - partnerId dengan simple quote does not cause server error / partnerId with simple quote does not cause server error
- Location: tests\api\get-token\get-token.contract.spec.ts:162:9

# Error details

```
Error: GT-SEC-002: HTTP status berdasarkan expected responseCode tidak sesuai. Expected: 400; Actual: 401 / GT-SEC-002: HTTP status based on expected responseCode mismatch. Expected: 400; Actual: 401
```
